    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS host_config_snapshot (
                ...
                sap_services_ports TEXT NOT NULL DEFAULT '-',
                updated_at_utc TEXT NOT NULL
            )
            """
        )
        ...
        if "sap_services_ports" not in existing_host_config_snapshot_columns:
        ...
        if "host_uid" not in existing_muted_alert_columns:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS muted_alert_rules_v2 (
                    host_uid TEXT NOT NULL,
                    hostname TEXT NOT NULL,
                    mountpoint TEXT NOT NULL,
                    muted_by TEXT NOT NULL DEFAULT '',
            ...
            conn.execute(
                """
                INSERT OR IGNORE INTO muted_alert_rules_v2 (host_uid, hostname, mountpoint, muted_by, muted_at_utc)
                SELECT COALESCE(NULLIF(hostname, ''), ''),
                       COALESCE(hostname, ''),
                       COALESCE(mountpoint, ''),
                       COALESCE(muted_by, ''),
                       COALESCE(muted_at_utc, '')
                FROM muted_alert_rules
                """
            )
            conn.execute("DROP TABLE muted_alert_rules")
            conn.execute("ALTER TABLE muted_alert_rules_v2 RENAME TO muted_alert_rules")
        conn.execute(
            """
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS filesystem_visibility (
                ...
                mountpoint TEXT NOT NULL,
                updated_at_utc TEXT NOT NULL,
                PRIMARY KEY(username, host_uid, section, mountpoint),
                FOREIGN KEY(username) REFERENCES web_users(username)
            )
            """
        )
        existing_visibility_columns = {
            str(row[1])
            for row in conn.execute("PRAGMA table_info(filesystem_visibility)").fetchall()
        }
        if "host_uid" not in existing_visibility_columns:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS filesystem_visibility_v2 (
                    username TEXT NOT NULL,
                    host_uid TEXT NOT NULL,
                    hostname TEXT NOT NULL,
                    section TEXT NOT NULL,
            ...
            conn.execute(
                """
                INSERT OR IGNORE INTO filesystem_visibility_v2 (username, host_uid, hostname, section, mountpoint, updated_at_utc)
            ...
            conn.execute("DROP TABLE filesystem_visibility")
            conn.execute("ALTER TABLE filesystem_visibility_v2 RENAME TO filesystem_visibility")
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_filesystem_visibility_user_host_uid_section
            ON filesystem_visibility(username, host_uid, section)
            """
        )
        conn.execute(
            """
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS backup_automation_settings (
                ...
                local_retention_days INTEGER NOT NULL DEFAULT 3,
                local_retention_max_files INTEGER NOT NULL DEFAULT 4,
        ...
        backup_automation_columns = {
            str(row[1] or "")
            for row in conn.execute("PRAGMA table_info(backup_automation_settings)").fetchall()
        }
        if "local_retention_max_files" not in backup_automation_columns:
            conn.execute(
                """
                ALTER TABLE backup_automation_settings
                ADD COLUMN local_retention_max_files INTEGER NOT NULL DEFAULT 4
                """
            )
        conn.execute(
            """
...
def get_backup_automation_settings(conn: sqlite3.Connection) -> dict[str, object]:
    ...
    if not row:
        ...
        conn.execute(
            ...
            (
                ...
                AUTO_BACKUP_DEFAULT_INTERVAL_HOURS,
                AUTO_BACKUP_DEFAULT_RETENTION_DAYS,
                AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES,
                AUTO_BACKUP_DIR.name,
                now_utc,
            ),
        )
    ...
    return {
        ...
        "local_retention_max_files": _coerce_int(
            row[3],
            AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES,
            0,
            20,
        ),
        "local_target_dir": _normalize_backup_target_dir(row[4]),
        "sftp_enabled": bool(int(row[5] or 0)),
        "sftp_host": str(row[6] or ""),
        "sftp_port": _coerce_int(row[7], 22, 1, 65535),
        "sftp_username": str(row[8] or ""),
        "sftp_remote_path": str(row[9] or ""),
        "sftp_auth_mode": "password" if str(row[10] or "").strip().lower() == "password" else "key",
        "sftp_key_path": str(row[11] or ""),
        "sftp_password": str(row[12] or ""),
...
def save_backup_automation_settings(conn: sqlite3.Connection, payload: dict) -> dict[str, object]:
    ...
    local_retention_max_files = _coerce_int(
        payload.get(
            "local_retention_max_files",
            existing.get("local_retention_max_files", AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES),
        ),
        AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES,
        0,
        20,
    ...
    conn.execute(
        ...
        ON CONFLICT(id) DO UPDATE SET
            ...
            local_retention_max_files = excluded.local_retention_max_files,
...
def _prune_local_automated_backups(
    target_dir: Path,
    *,
    max_files: int,
    retention_days: int,
    ...
    for index, (mtime, path) in enumerate(entries):
        if index >= max_files or mtime < cutoff:
            try:
                path.unlink()
                pruned_count += 1
            except OSError:
...
def _run_local_automated_backup(settings: dict[str, object], trigger_source: str) -> dict[str, object]:
    ...
    retention_max_files = _coerce_int(
        settings.get("local_retention_max_files", AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES),
        AUTO_BACKUP_DEFAULT_RETENTION_MAX_FILES,
        0,
        20,
    )
    ...
    pruned_count = _prune_local_automated_backups(
        target_dir,
    ...
    backup_path_value = str(backup_file.relative_to(DATA_DIR))
    ...
    return {
        ...
        "local_retention_max_files": retention_max_files,
...
def save_filesystem_visibility_hidden(
    ...
    conn.execute(
        "DELETE FROM filesystem_visibility WHERE username = ? AND host_uid = ? AND section = ?",
        (username, host_key, section),
    )
    now_utc = utc_now_iso()
    for mountpoint in normalized:
        conn.execute(
            """
            INSERT INTO filesystem_visibility (username, host_uid, hostname, section, mountpoint, updated_at_utc)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (username, host_key, hostname, section, mountpoint, now_utc),
        )
    return normalized
        ...
        for batch in _iter_changelog_report_batches(
            ...
            for row in batch:
                ...
                if report_count == 1 or report_count % pulse_every == 0 or reports_remaining <= 50:
                    ...
                    if inventory_greenfield and report_count in {1, 10, 100, 500}:
                        print(
                            f"[inventur] job={job_id} config {report_count}/{reports_total} "
                            f"changes={inserted_changes} host={hostname}",
                            flush=True,
                        )
...
def collect_system_overview(conn: sqlite3.Connection, search_query: str = "") -> dict:
    ...
    host_uid_settings_map: dict[str, dict[str, str]] = {}
    if host_uid_keys:
    ...
    for row in latest_rows:
        ...
        country = (
            ...
            or extract_country_code_from_payload(payload)
            or "XX"
        )
...
def collect_customer_overview(conn: sqlite3.Connection) -> dict:
    ...
    alert_counts_rows = conn.execute(
        ...
        WHERE NOT EXISTS (
          SELECT 1 FROM muted_alert_rules m
                    WHERE m.host_uid = COALESCE(NULLIF(a.host_uid, ''), a.hostname)
    ...
    for row in latest_rows:
        ...
        alert_counts = alert_counts_by_host.get(host_uid or hostname, {"open": 0, "critical": 0})
...
def branded_info_mail_html(
    ...
    return (
        ...
        f"<div style='padding:0 20px 16px 20px;'>{mail_footer_logos_html(ang_logo_uri)}</div>"
        "</div>"
        "</body></html>"
    )
...
def trend_digest_html(username: str, warnings: list[dict], hours: int) -> str:
    ...
    return (
        ...
        "</table>"
        f"{mail_footer_logos_html(ang_logo_uri)}"
        "</div>"
        "</div>"
        "</body></html>"
    )
...
def mail_footer_logos_html(ang_logo_uri: str) -> str:
    ang_logo_html = (
...
def delete_host_card_data(conn: sqlite3.Connection, hostname: str, host_uid: str = "") -> dict[str, int]:
    ...
    if normalized_host_uid:
        ...
        conn.execute(
            "DELETE FROM alerts WHERE COALESCE(NULLIF(host_uid, ''), hostname) = ?",
            (normalized_host_uid,),
        )
        row = conn.execute("SELECT changes()").fetchone()
        deleted["alerts"] += int(row[0] or 0) if row else 0
...
def update_alerts_for_report(conn: sqlite3.Connection, hostname: str, host_uid: str, report_id: int, filesystems: list, alarm_settings: dict) -> None:
    ...
    for fs in filesystems:
        ...
        if is_alert_muted(conn, alert_key, mountpoint):
...
"""
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/v1/host-settings":
            ...
            self._send_json(
                HTTPStatus.OK,
                {
                    ...
                    "customer_it_provider_name": host_settings.get("customer_it_provider_name", ""),
                    "customer_it_provider_contact": host_settings.get("customer_it_provider_contact", ""),
                    "customer_it_provider_email": host_settings.get("customer_it_provider_email", ""),
                    "customer_it_provider_phone": host_settings.get("customer_it_provider_phone", ""),
                    "customer_it_provider_name_2": host_settings.get("customer_it_provider_name_2", ""),
                    "customer_it_provider_contact_2": host_settings.get("customer_it_provider_contact_2", ""),
                    "customer_it_provider_email_2": host_settings.get("customer_it_provider_email_2", ""),
                    "customer_it_provider_phone_2": host_settings.get("customer_it_provider_phone_2", ""),
                    "customer_it_provider_name_3": host_settings.get("customer_it_provider_name_3", ""),
                    "customer_it_provider_contact_3": host_settings.get("customer_it_provider_contact_3", ""),
                    "customer_it_provider_email_3": host_settings.get("customer_it_provider_email_3", ""),
                    "customer_it_provider_phone_3": host_settings.get("customer_it_provider_phone_3", ""),
                    "customer_it_provider_contacts": host_settings.get("customer_it_provider_contacts", []),
                },
            )
            return
        ...
        if parsed.path == "/api/v1/host-update-log":
            ...
            host_uid_param = (query.get("host_uid") or [""])[0].strip()
            if not hostname_param and not host_uid_param:
                self._send_json(HTTPStatus.BAD_REQUEST, {"error": "hostname or host_uid parameter required"})
                return
            try:
                with sqlite3.connect(DB_PATH) as conn:
                    host_key_expr = reports_host_key_sql()
                    where_clause = "hostname = ?"
                    where_args: tuple = (hostname_param,)
                    if host_uid_param:
                        where_clause = f"({host_key_expr} = ?)"
                        where_args = (host_uid_param,)

                    row = conn.execute(
                        f"SELECT payload_json, COALESCE(hostname, '') FROM reports WHERE {where_clause} ORDER BY id DESC LIMIT 1",
                        where_args,
                    ).fetchone()
                if not row:
                    self._send_json(HTTPStatus.OK, {"hostname": hostname_param, "host_uid": host_uid_param, "lines": [], "crash_info": "", "available": False})
                    return
                resolved_hostname = str(row[1] or "").strip() or hostname_param
                payload = {}
                if row[0]:
                    try:
        ...
        if parsed.path == "/api/v1/alert-mutes":
            with sqlite3.connect(DB_PATH) as conn:
                rows = conn.execute(
                    "SELECT host_uid, hostname, mountpoint, muted_by, muted_at_utc FROM muted_alert_rules ORDER BY hostname, mountpoint"
                ).fetchall()
            self._send_json(
                HTTPStatus.OK,
                {
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/v1/alert-mute":
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                conn.execute(
                    "INSERT OR REPLACE INTO muted_alert_rules (host_uid, hostname, mountpoint, muted_by, muted_at_utc) VALUES (?, ?, ?, ?, ?)",
